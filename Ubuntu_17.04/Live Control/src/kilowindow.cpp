...
KiloWindow::KiloWindow(QWidget *parent): QWidget(parent), device(2), sending(false), connected(false) {
    // Create status bar
    ...
    // create UPD form window
    form = new Form;
    dynamic_cast<Form*>(form)->setKiloWindow(this);
}
...
QGroupBox *KiloWindow::createFileInput() {
    ...
    QLabel *myIp_label1 = new QLabel("My IP is:");
    QLabel *myIp_label2 = new QLabel("Ip here");
    udpIp = new QLineEdit("");
    udpIp->setPlaceholderText("IP address");
    QPushButton *sendUdp_button = new QPushButton("Connect");
    new_button = new QPushButton("Mental Commands");
    ...
    QObject::connect(new_button, SIGNAL(clicked()), this, SLOT(mentalCommands()));
    QObject::connect(sendUdp_button, SIGNAL(clicked(bool)), this, SLOT(sendUdp()));
    ...
    layout->addWidget(myIp_label1, 3, 0);
    layout->addWidget(myIp_label2, 3, 1);
    layout->addWidget(udpIp, 4, 0);
    layout->addWidget(sendUdp_button, 4, 1);
    layout->addWidget(new_button, 5, 0, 1, 2);

    //IP-------------
    QList<QHostAddress> list = QNetworkInterface::allAddresses();
    myIp_label2->setText(list[2].toString());
    //end Ip---------
...
void KiloWindow::sendDataMessage(uint8_t *payload, uint8_t type) {
    ...
    if (device == 0){
        ftdi_conn->sendCommand(packet);
//        qDebug() << "I am to main1" << type;

}
    else if (device == 1){
        vusb_conn->sendCommand(packet);
//        qDebug() << "I am to main2" << type;

    }
    else{
        serial_conn->sendCommand(packet);
//        qDebug() << "I am to main3" << type;
    }
}

//------------------------------My Method-------------------------------------------------------------
void KiloWindow::mentalCommands() {
    socket->close();
    socket->disconnect();

    dynamic_cast<Form*>(form)->show();
...
void KiloWindow::sendUdp(){

    QByteArray Data;
    Data.append("Hello from Ubuntu");
    if(udpIp->text() != ""){
        ip = udpIp->text();
    }
...
void   KiloWindow::closeEvent(QCloseEvent*){
    qApp->quit();
}
